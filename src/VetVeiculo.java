...
// Declaração da classe VetVeiculo
public class VetVeiculo {
    ...
    public boolean insereVeiculo(Veiculo veiculo) {
        for (Veiculo v : veiculos) { // Itera sobre o ArrayList para verificar se a placa já existe
    ...
    public int pesquisaVeiculo(String placa) {
        for (int i = 0; i < veiculos.size(); i++) { // Itera sobre o ArrayList
            if (veiculos.get(i).getPlaca().equals(placa)) { // Verifica se a placa corresponde
    ...
    public Veiculo getPos(int posicao) {
        if (posicao >= 0 && posicao < veiculos.size()) { // Verifica se a posição é válida
    ...
    public boolean removeVeiculo(String placa) {
        for (Veiculo v : veiculos) { // Itera sobre o ArrayList
            if (v.getPlaca().equals(placa)) { // Verifica se a placa corresponde
